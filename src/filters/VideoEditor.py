...
class VideoEditor(Filter):
    ...
    def apply(self, frames: np.ndarray, processes_limit: int, pool: Pool, num_frames: int, width: int, height: int):
        print("Start VideoEditor")
        output = np.empty((num_frames, height, width, 3), np.uint8) # Создаем список кадров
        index = 0
        for frame in frames:
            frame = self.filter.apply(self, frame, processes_limit, pool) # Приемняем фильтр к каждому кадру
            output[index, :, :, :] = frame[0] # Обязательно [0]
            index += 1
        return output
